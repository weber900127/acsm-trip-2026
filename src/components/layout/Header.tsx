...
import { Plane, Calendar, Download, Upload, CreditCard, LogOut } from 'lucide-react';
...
export default function Header({ isEditing, onToggleEdit, onExport, onImport, onOpenWallet, user, onLogin, onLogout }: HeaderProps) {
    const [timeLeft, setTimeLeft] = useState({ days: 0, hours: 0, minutes: 0 });
    const fileInputRef = useRef<HTMLInputElement>(null);

    useEffect(() => {
        const calculateTimeLeft = () => {
            const tripDate = new Date('2026-05-20T00:00:00');
            const now = new Date();

            setTimeLeft({
                days: differenceInDays(tripDate, now),
                hours: differenceInHours(tripDate, now) % 24,
                minutes: differenceInMinutes(tripDate, now) % 60
            });
        };
                ...
                <div className="flex justify-between items-start">
                    <div>
                        <div className="flex items-center gap-2 mb-2 opacity-80">
                            ...
                            <span className="text-xs">2026.05.20 - 06.04</span>
                        </div>
